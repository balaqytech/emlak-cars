<header x-data="{ mobileMenuIsOpen: false }" x-on:click.away="mobileMenuIsOpen = false"
    class=" w-full bg-white text-sm py-3 border-b border-gray-200 z-50 shadow-3xl {{-- fixed top-0 left-0 right-0 --}}">
    <nav class="wrapper flex items-center justify-between gap-4">
        ...
        <ul class="hidden items-center md:flex gap-8 justify-center font-bold">
            ...
            <x-nav-link link="{{ app()->getLocale() == 'ar' ? '/en' : '/ar' }}"
                title="{{ app()->getLocale() == 'ar' ? 'English' : 'عربي' }}" />
        </ul>
        <div class="flex items-center gap-4">
            <x-primary-button href="{{ localizedUrl('/contact') }}"
                class="hidden md:block">{{ __('frontend.navigation.contact') }}</x-primary-button>
            <x-search-modal />
        </div>
        <div class="z-50">
            {{-- mobile menu button --}}
