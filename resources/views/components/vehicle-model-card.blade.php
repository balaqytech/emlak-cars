<div class="flex flex-col gap-4 p-4 bg-white shadow-3xl rounded-2xl">
    <a href="/vehicles/{{ $model->vehicle->slug }}/{{ $model->id }}" class="relative w-full h-64 bg-cover bg-center bg-no-repeat rounded-xl overflow-hidden"
    ...
    <div class="flex flex-col gap-4 p-4">
        <h3 class="text-xl font-semibold text-primary">
            <a href="/vehicles/{{ $model->vehicle->slug }}/{{ $model->id }}" class="hover:underline">{{ $model->name }}</a>
        </h3>
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <x-primary-button href="/vehicles/{{ $model->vehicle->slug }}/{{ $model->id }}"
                class="text-sm text-primary hover:underline">{{ __('frontend.vehicles.view_details') }}</x-primary-button>
                <x-outline-button href="/vehicles/{{ $model->vehicle->slug }}/{{ $model->id }}#contact"
