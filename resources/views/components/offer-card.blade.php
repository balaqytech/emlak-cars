<article
    class="flex flex-col bg-white shadow-3xl rounded-lg border border-slate-200 overflow-hidden transition-all duration-500">
    <a href="/offers/{{ $offer->slug }}" class="relative overflow-hidden h-80">
    ...
    <div class="p-10 flex flex-col gap-2 bg-white">
        <h3 class="text-xl font-bold text-slate-700">
            <a href="/offers/{{ $offer->slug }}" class="hover:underline hover:text-primary">{{ $offer->title }}</a>
        </h3>
    ...
    <div class="relative flex flex-col gap-2 px-10 py-4">
        <p class="text-gray-600 text-sm flex items-center gap-2">
            ...
            <time
                datetime="{{ $offer->due_date }}">{{ __('frontend.due_date_left', ['date' => $offer->due_date->diffForHumans()]) }}</time>
        </p>
