<article
    ...
    <a href="{{ localizedUrl('/posts/' . $post->slug) }}" class="relative h-80">
        @if ($post->is_featured)
    ...
    <div class="p-10 flex flex-col gap-2 grow">
        <h3 class="text-xl font-bold text-slate-700">
            <a href="{{ localizedUrl('/posts/' . $post->slug) }}" class="hover:underline hover:text-primary">{{ $post->title }}</a>
        </h3>
        ...
        <a href="{{ localizedUrl('/posts/' . $post->slug) }}"
